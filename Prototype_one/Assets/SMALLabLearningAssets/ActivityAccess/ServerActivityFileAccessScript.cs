...
public class ServerActivityFileAccessScript : MonoBehaviour {

    [Header("ScriptVersion 3.2")]
    [Space(10)]
    ...
    void handleServerResponseFromGetActivityFileData(bool success, string errorMessage, bool connectedToLicenseServer, JSONObject data)
    {


        Debug.Log("Logging Success: " + success + ", Error: " + errorMessage + ", Server Connected: " + connectedToLicenseServer);

        configFilePanelScript.HandleActivityFileDataFromServer(success, errorMessage, connectedToLicenseServer, data);
    ...
    public void GetActivityFileDataFromServer(int activityFileId)
    {
        //string activityFileName = Path.GetFileNameWithoutExtension(activityFilePath);

        Debug.Log("Getting activity file data for = " + activityFileId);
        ...
        j.AddField("activityfile_id", activityFileId);
        ...
        {
            StartCoroutine(WaitForPreviousProcessToFinish(jsonEncodedString));
        }

    }

    IEnumerator WaitForPreviousProcessToFinish(string jsonEncodedString)
    {
        ...
        StartCoroutine(CheckLicenseServerConnection(
                ...
                { // first, make sure that the machine can connect to the license server
                    if (isConnected)
                    {
                        ...
                        StartCoroutine(MakeRequestToGetActivityFileData(jsonEncodedString)); // if it can proceed
                    }
                    else
                    {
                        handleServerResponseFromGetActivityFileData(false, "Unable to connect to server", false, null);
                    }
                }
            ));
    }

    public IEnumerator MakeRequestToGetActivityFileData(string jsonEncodedString)
    {

        Dictionary<string, string> postHeader = new Dictionary<string, string>();
        postHeader.Add("Content-Type", "application/json");

        this.serverRequest = new WWW(scenarioAnalyticsScript.serverBaseUrl + "/api/v1/activities/get_activityfile_data?", System.Text.Encoding.UTF8.GetBytes(jsonEncodedString), postHeader);
        ...
        handleServerResponseFromGetActivityFileData(statusObj.GetField("success").b, errorObj.str, true, contentObj);
